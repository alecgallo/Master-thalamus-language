...
summary_vars <- function(d,n_vars=c(),c_vars=c()) {
  summary_n_vars<- apply(d[,n_vars],2,function(x){
    data.frame(
      n=length(!is.na(x)),
      n_NA=sum(is.na(x)),
      range=paste(min(x,na.rm=TRUE)%>% round(.,digits=3),max(x,na.rm=TRUE)%>% round(.,digits=3),sep=";") ,
  ...
  summary_vars<-merge(summary_n_vars,counts_c_vars,all=TRUE,stringsAsFactors=FALSE,by=c("variable","n","n_NA"))
  summary_vars<-summary_vars %>% select(variable,n,n_NA,levels,counts,range,median,mean,sd,skewness,kurtosis) %>% arrange(levels,variable)
