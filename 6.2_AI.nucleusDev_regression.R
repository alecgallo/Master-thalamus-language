...
#--------------------------------------
for (dv in thal_cols_of_interest2){
  ...
  if (dv != "AI.MGN") {
    plot_model(m_final, type = "std") %>%
      ggsave(., file = paste(out_dir, "/", dv, "_m_final", "_std_sjplot", ".png", sep = ""))
  }
  #
  ...
  # save summary tables
  write.csv(anova_table,file=paste0(out_dir,"/tables/",dv,"_final_","anova_table_AI.csv"),row.names = TRUE)
  write.csv(effect_table,file=paste0(out_dir,"/tables/",dv,"_final_","effect_r2m_table_AI.csv"),row.names = FALSE)
  write.csv(table_m1, file=paste0(out_dir,"/tables/",dv,"_m1_","summary_table_AI.csv"), row.names = TRUE)
  ...
  write.csv(estimates_df, file=paste0(out_dir,"/tables/estimatesAI_table.csv"), row.names = FALSE)
  write.csv(estimates0_df, file=paste0(out_dir,"/tables/estimates0AI_table.csv"), row.names = FALSE)
  #--------------------------------------
  # clean intermediate
...
p<-plot_model(m1,terms=c("age","sex"),type="pred")

p <- p + xlab("Age") + ylab("AI Total Pulvinar") + 
  ggtitle("Asymmetry index of the Total Pulvinar")
