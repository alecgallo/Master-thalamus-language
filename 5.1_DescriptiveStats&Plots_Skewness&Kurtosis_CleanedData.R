...
#adjusted_volumes <- paste0("adj_", thal_cols) %in% colnames(data) #AG: this should be excluded because in the cleaned data we only have the raw volumes, right?
#------------------
...
# save
write.csv(descriptives,file="summary_tables/ThalamicNuclei_descriptives_cleaned.csv")
write.csv(descriptives2,file="summary_tables/ThalamicNuclei_descriptives_cleaned2.csv",row.names=FALSE)
# openxlsx::write.xlsx(descriptives,file="summary_tables/AdjustedthalamicNuclei_descriptives.xlsx")

################################
#1. Descriptive Statistics of adjusted thalamic nuclei (without outliers!)
descriptives <- data %>%
  group_by(timepoint) %>%
  skimr::skim_without_charts(paste0(thal_cols),age)
# summarise(across(all_of(thal_cols)),list(min = min(), max = max)) # not working, could keep trying
colnames(descriptives)<-gsub("numeric.","",colnames(descriptives))
# 
write.csv(descriptives,file="summary_tables/thalamicNuclei_descriptives_CleanedData.csv")
# openxlsx::write.xlsx(descriptives,file="summary_tables/thalamicNuclei_descriptives.xlsx")
#--------------------------------------

#1.1. Create descriptive statistics (maybe we can delete one of them: The skimr is finally working on my computer!)
## we can get rid of one of the two descriptive statistics as in script 2

# define aseg cols
aseg_cols<-colnames(aseg)[grep("subject|session|timepoint",colnames(aseg),invert=TRUE)] %>% make.names()

cols2check<-c(paste0(thal_cols))
...
write.csv(descriptive_df,file="summary_tables/descriptive_with4sd_Cleaned_data.csv")

##PLOTS
# Histograms of the raw thalamic nuclei

if (dir.exists("figures/histograms")==FALSE) { dir.create("figures/histograms") }
if (dir.exists("figures/histograms/rawnuclei_CleanedData")==FALSE) { dir.create("figures/histograms/adjustednuclei_CleanedData") }

# in order to access the content of descriptive_df, you can now use "volume" for filtering.
for (vol in thal_cols){ #AG: I have edited here as well, loop for the thal_cols, instead of "adj_", thal_cols
  #if (!file.exists("figures/histograms/rawnuclei_CleanedData/", plot_name, "_histogram.png")){ #not sure why It's not working
    ...
    plot_name<-paste("dist",vol,sep="_")
    assign(plot_name,p)
    # save as figure
    ggsave(p, file = paste0("figures/histograms/rawnuclei_CleanedData/", plot_name, "_histogram.png"),
